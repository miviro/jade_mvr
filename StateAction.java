...
class LearningTools {
  ...
  /**
   * This method is used to select the next action (Schaerf) considering a
   * statistical
   * criterium for the action that provided more benefits in the last
   * iSizeBufferStat attempts.
   *
   */
  ...
  /**
   * This method uses Learning Automata (LA) to select a new action depending on
   * the
   * past experiences. The algorithm works as: store, adjust and generate a new
   * action.
   * 
    ...
    if (oLastStateAction != null) { // Adjusting Probabilities
      if (dReward > 0) // If reward grows and the previous action was allowed --> reinforce last action
        for (int i = 0; i < iNActions; i++)
          if (i == iLastAction)
            oLastStateAction.dValAction[i] += dLearnRate * (1.0 - oLastStateAction.dValAction[i]); // Reinforce the last
                                                                                                   // action
          else
